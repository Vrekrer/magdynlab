...
class Trigger(object):
    ...
    def set_Analog_Trigger(self, trigType, lowThreshold=0,
        ...
        if trigType in self.analogTriggers:
            ...
            self._lowThreshold = ul.from_eng_units(
                self.parent.boardNum,
                self.parent.Channels[channel]._ULrange,
                lowThreshold
            )
            self._highThreshold = ul.from_eng_units(
                self.parent.boardNum,
                self.parent.Channels[channel]._ULrange,
                highThreshold
            )
...
class USB_1602HS(object):
    ...
    def Adquire(self):
        enabledChannels = _np.array(
            [ch.num for ch in self.Channels if ch.enabled],
            dtype=_np.int16
        )
        UL_gains = _np.array(
            [ch._ULrange for ch in self.Channels if ch.enabled],
            dtype=_np.int16
        )
        gains = _np.array(
            [ch._rangeVpp for ch in self.Channels if ch.enabled],
            dtype=_np.int16
        )
        ...
        trueRate = ul.a_in_scan(self.boardNum, 0, 0, dataSize,
                                self.rate, 0, rawData.ctypes.data, opts)
        voltsOut = _np.uint16(rawData) / 2.0**16 - 0.5
        voltsOut = voltsOut.reshape((self.nSamples, nChannels)).T
