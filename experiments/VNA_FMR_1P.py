...
@ThD.gui_safe
def Plot_dPdH_frequency(Data):
    f = plt.figure('VNA-FMR dP/dH - Frequency', (5, 4))
    ...
    line = ax.lines[-1]
    line.set_data(Data['f'], Data['dP/dH']*1000)
    ax.set_xlabel('Frequency (Oe)')
    ax.set_ylabel('dP/dH')
    ax.grid(True)
...
class VNA_FMR_1P(object):
    def __init__(self, ResouceNames={}):
        ...
        PowerSource = magdynlab.instruments.KEPCO_BOP(ResourceName=RN_Kepco,
                                                      logFile=logFile)
        VNA = magdynlab.instruments.RS_VNA_Z(ResourceName=RN_VNA,
                                             logFile=logFile)
    ...
    def Measure_dPdH(self, fields, freq, file_name, 
                     ...
                     hold_time=0.0, osc_hold_time=0.01):
