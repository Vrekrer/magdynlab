...
class VNA_FMR(object):
    def __init__(self, GPIB_Device=0):
        ...
        class virtual_voltmeter(object):
            def __init__(self):
                K2400 = magdynlab.instruments.KEITHLEY_2400(GPIB_Device=GPIB_Device, logFile=logFile)
                self.K2400 = K2400
                self.K2400.write('FUNC "VOLT:DC"')
                self.K2400.write('VOLT:DC:RANG:UPP 1')
                self.K2400.write('VOLT:DC:AVER:TCON MOV')
                self.K2400.write('VOLT:DC:AVER:COUN 20')
                self.K2400.write('VOLT:DC:DIG 5')
                self.K2400.write('INIT:CONT 1')
            @property
            def voltage(self):
                '''Voltage Value'''
                return self.K2400.query_float('FETCH?')

        Voltmeter = virtual_voltmeter()
